let BASE_URL = 'http://localhost:5000/api/enhance';
...
function getUrl(option, content) {
  ...
  switch (option.type) {
    ...
    case 'spelling':
      ...
      break;
    case 'lucky':
      url = `${BASE_URL}?action=lucky&content=${encodeURIComponent(content)}`;
      break;
    default:
      break;
  }
...
function addMenu(parent, inputElement, options) {
  options.forEach(function (option) {
    ...
    if (option.type == 'group') {
      ...
      addMenu(submenu, inputElement, option.children);
    }
  });
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function enhanceInputElement(inputElement) {
    ...
    // Create a menu wrapper div to add the AI menu and button inside
    ...
    addMenu(aiMenu, inputElement, options);

    // Append the AI menu to the wrapper div
    menuWrapper.appendChild(aiMenu);

    // Append the AI button inside the menu wrapper div
    menuWrapper.appendChild(aiButton);

    menuWrapper.className = 'micro-ai-menu-wrapper';

    // Append the menu wrapper to the wrapper div
